...
class DottedDetection(Node):
    def __init__(self):
        super().__init__('dotted_detection')
        #Create Publishers
        self.dotted_pub = self.create_publisher(Int32, '/dotted', 10)
        #Create Subscribers 
        self.frame_sub = self.create_subscription(Image, '/dotted_frame', self.subscriber_callback, 10)
        self.get_logger().info('dotted_detection Initialized')
    ...
    def subscriber_callback(self, IMG):
        ...
        # frames = cv2_conc([img, img_weighted, thresh, thresh_tmp])
        # cv2.imshow('frames', frames)
        # cv2.waitKey(1)

        # self.get_logger().info(f'black areas: {clusters}')

        if clusters > 3:
            self.dotted.data = 1
        else:
            self.dotted.data = 0
        self.dotted_pub.publish(self.dotted)
