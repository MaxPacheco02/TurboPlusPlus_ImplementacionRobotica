...
public:
  ...
  {
    // Set weight params for different performances based on the scenario.
    ...
    auto package_share_directory = boost::filesystem::path(
        ament_index_cpp::get_package_share_directory("pzb_control"));
    ...
    velocity_sub_ = this->create_subscription<geometry_msgs::msg::Vector3>(
        "vel", 10, [this](const geometry_msgs::msg::Vector3 &msg)
        {
          state_->u_cmd = msg.x;
          state_->r_cmd = msg.z; });
    ...
    pose_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
        ...
        [this](const nav_msgs::msg::Odometry &msg)
        {
          ...
          state_->yaw = tf2::getYaw(msg.pose.pose.orientation);
        });

    mpc_goal_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
        "mpc/goal_pose", 10,
        [this](const geometry_msgs::msg::PoseStamped &msg)
        {
          first_goal_received = true;

          base_wp.x = pose.position.x;
          base_wp.y = pose.position.y;
          next_wp.x = msg.pose.position.x;
          next_wp.y = msg.pose.position.y;
          next_wp.theta = tf2::getYaw(msg.pose.orientation);
        });

    obstacle_list_sub_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
    ...
    pose_setpoint_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>(
        "goal_pose", 10);

    xe_debug_pub_ = this->create_publisher<std_msgs::msg::Float64>(
        "mpc/debug/xe", 10);
...
private:
  ...
  rclcpp::Subscription<geometry_msgs::msg::PoseStamped>::SharedPtr mpc_goal_sub_;
  rclcpp::Subscription<std_msgs::msg::Float64MultiArray>::SharedPtr obstacle_list_sub_;

  rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr pose_setpoint_pub_;
  rclcpp::Publisher<std_msgs::msg::Float64>::SharedPtr xe_debug_pub_, ye_debug_pub_, psie_debug_pub_, obs_cost_debug_pub_;
  ...
  std_msgs::msg::Float64 xe_debug_msg, ye_debug_msg, psie_debug_msg, obs_cost_debug_msg;
  std_msgs::msg::Float64MultiArray qs_debug_msg;
  nav_msgs::msg::Path mpc_projection_path_msg;

  geometry_msgs::msg::Pose pose;
  geometry_msgs::msg::PoseStamped pose_setpoint_msg;

  std::optional<MPC_State> state_;

  std::array<double, 10> obs_arr{100., 100., 100., 100., 100., 100., 100., 100., 100., 100.};
  ...
  double integral_step{0.1};

  bool first_goal_received{false};

  void update()
  {
    if (!state_.has_value() || !first_goal_received)
    {
      return;
    }
    ...
    RCLCPP_INFO(this->get_logger(), "Base waypoint: %f, %f", base_wp.x, base_wp.y);
    RCLCPP_INFO(this->get_logger(), "Next waypoint: %f, %f, %f", next_wp.x, next_wp.y, next_wp.theta);

    auto target_setter = app_->get_parameter_setter("target");
    target_setter.set_value({base_wp.x, base_wp.y, next_wp.x, next_wp.y, next_wp.theta});

    RCLCPP_INFO(this->get_logger(), "Setting target");
    RCLCPP_INFO(this->get_logger(), "%f,%f,%f", next_wp.x, next_wp.y, next_wp.theta);
    ...
    RCLCPP_INFO(this->get_logger(), "Qs (costs)");
    RCLCPP_INFO(this->get_logger(), "%f,%f,%f, %f,%f,%f",
                desired_weights[0], desired_weights[1], desired_weights[2],
                desired_weights[3], desired_weights[4], desired_weights[5]);
    ...
    pose_setpoint_msg.header.frame_id = "odom";
    mpc_projection_path_msg.header.frame_id = "odom";
    ...
    pose_setpoint_msg.pose.position.x = x_result[0];
    pose_setpoint_msg.pose.position.y = y_result[0];
    tf2::Quaternion tmp_q;
    tmp_q.setRPY(0, 0, psi_result[0]);
    pose_setpoint_msg.pose.orientation = tf2::toMsg(tmp_q);
    ...
    pose_setpoint_pub_->publish(pose_setpoint_msg);

    xe_debug_msg.data = xe_result[0];
    ye_debug_msg.data = ye_result[0];
    psie_debug_msg.data = psie_result[0];
    obs_cost_debug_msg.data = obs_cost_result[0];

    xe_debug_pub_->publish(xe_debug_msg);
  ...
  double obj_dist(double obj_x, double obj_y, geometry_msgs::msg::Pose p)
  {
    return sqrt(
        (obj_x - p.position.x) * (obj_x - p.position.x) +
        (obj_y - p.position.y) * (obj_y - p.position.y));
  }
