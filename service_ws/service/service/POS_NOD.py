...
class ObjPos(Node):
    ...
    def show(self):
        ...
        while True:
            ...
            if(self.cont == True):
                # ARU DISTANCIA
                frame, PUV, aru_ROT, aru_TRA  = self.aru.pose_estimation(frame,self.cam.CamMat,self.cam.CamDis)
                # d = self.point.aruDistance(PUV)

                # CALCULAR ORIENTACION DE ARUCO EN EL MUNDO
                self.aruORI, self.aruPOS = self.pos.aruOriToWorld(aru_ROT,aru_TRA)
