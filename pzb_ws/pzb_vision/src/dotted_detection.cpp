...
class DottedDetection : public rclcpp::Node
{
  public:
    DottedDetection() : Node("dotted_detection") {
        dotted_pub_ = this->create_publisher<std_msgs::msg::Int32>("dotted", 10);
        frame_sub_ = this->create_subscription<sensor_msgs::msg::CompressedImage>("floor_frame", 10, 
                        std::bind(&DottedDetection::subscriber_callback, this, _1));
  ...
  private:
    ...
    rclcpp::Publisher<std_msgs::msg::Int32>::SharedPtr dotted_pub_;
    rclcpp::Subscription<sensor_msgs::msg::CompressedImage>::SharedPtr frame_sub_;

    std_msgs::msg::Int32 dotted_msg;
    ...
    void subscriber_callback(const sensor_msgs::msg::CompressedImage & msg) {
      ...
      // my Pure thresh
      Calc result = pure_thresh(thresh);
    ...
    Calc pure_thresh(cv::Mat mat){
      ...
      calc.frame = pure_img;
      calc.clusters = windows_pure.size();
      return calc;
    }
};
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<DottedDetection>());
  rclcpp::shutdown();
  return 0;
}